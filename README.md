...
- **Actions**: everything real-world — Sheets/Calendar, WhatsApp sends, scheduled
  ...
  (`mcp-workspace-server`, `mcp-whatsapp-server`, `mcp-scheduler-server`).
  `mcp-whatsapp-server` never touches WhatsApp itself; it queues a Firestore
  command the extension executes, since only the extension holds the live session.
...
- `MCP_WORKSPACE_URL`, `MCP_WHATSAPP_URL`, `MCP_SCHEDULER_URL` — where those
  three servers are reachable (defaults assume all three running locally).

The `mcp-scheduler-server` additionally needs `GCP_PROJECT`, `SCHEDULER_LOCATION`,
and `INSTRUCT_FUNCTION_URL` (the deployed `/instruct` endpoint it POSTs back to
when a reminder fires) — set these in its own environment when you run/deploy it.

### 4. Firestore rules, indexes, and the vector index

```
firebase deploy --only firestore:rules,firestore:indexes
```

The vector index on `users/{uid}/memories.embedding` (768 dims, matching
`text-embedding-004`) is defined in `firestore.indexes.json` — this step is what
actually provisions it.

### 5. Run the MCP servers locally

Each is a standalone Node/Express process:
...
They default to ports 4001/4002/4003 (override with `PORT`). `mcp-workspace-server`
needs a connected Google OAuth token per user before `sheet_update`/`calendar_event`
will work — see the `TODO` in `mcp-workspace-server/src/googleAuth.ts` (Secret
Manager wiring isn't implemented yet, only stubbed).
...
hook. Point the extension's `API_BASE_URL` (`extension/src/config.ts`) at the
emulator's local URL while testing this way, and set `FIRESTORE_EMULATOR_HOST`
in any local script's environment to hit the same emulated Firestore.
...
→ select `extension/dist`. Before building, fill in `extension/src/firebaseClient.ts`
with your Firebase web app config (from the Firebase console → Project settings).
...
`extension/src/whatsappAdapter.ts` is stubbed out — WhatsApp Web's DOM is
unofficial and changes often, so this is intentionally left as a TODO to fill in
against the live page rather than guessed at. Everything else (message routing,
auth, the command queue) is wired and ready for it.

## Offline testing (no live WhatsApp session needed)

Useful once the DOM adapter above is filled in, or for testing the server-side
logic against a manually-crafted dump:
...
environment config) with their Cloud Run URLs.
